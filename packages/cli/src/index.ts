...
import { sendTelegramMessage } from "@oiesooujeje/sendkit-core";
...
function getTelegramBotToken() {
  if (!existsSync(configPath)) {
    throw new Error(
      `Config file not found at ${configPath}. Please run 'sendkit config telegram <botToken>' to set your Telegram bot token.`,
    );
  }
  ...
  if (!token) {
    throw new Error(
      `Telegram bot token not found in config file at ${configPath}. Please run 'sendkit config telegram <botToken>' to set your Telegram bot token.`,
    );
  }
...
program.name("sendkit").description("SendKit CLI backed by JEJE");
